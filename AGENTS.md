...
- `src/widgets/Summary` is the shared order-summary sidebar. It never reads a store directly — screens read their own store (`title`/`price`/etc, e.g. `QuotationStore`) and pass it as props. Accumulating detail sections work differently: add one entry to `SUMMARY_SECTIONS_BY_STEP` and one entry to `sectionsByStep` in `src/widgets/Summary/summarySections.tsx`, keyed by `FORM_NAMES` — do not invent a separate step enum for this.
