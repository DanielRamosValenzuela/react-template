...
component, check `.github/instructions/tomaco-component-spacing.md` for that
component's own baked-in default first — a Figma-measured value is the
rendered result, which already includes what the component provides by
default. This file also has the exact reason a raw Playwright `.click()` can
misfire on `CheckBox`/`RadioButton`/`Switch`, and `Dialog`'s real overflow
behavior.
